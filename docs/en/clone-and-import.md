...
`git clone https://github.com/ByteLegendQuest/java-fix-integer-overflow`

A project directory named `java-fix-integer-overflow` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-integer-overflow` directory which is created just now.
